...
import typing
from .v3 import MetadataValidatorV3
from .common import BaseMetadataValidator, ValidationError
from nbformat.notebooknode import NotebookNode


class MetadataValidatorV4(BaseMetadataValidator):

    schema_version = 4

    def __init__(self) -> None:
        super().__init__()
        self.v3 = MetadataValidatorV3()

    def _upgrade_v3_to_v4(self, cell: NotebookNode) -> NotebookNode:
        meta = cell.metadata['nbgrader']
        meta['schema_version'] = self.schema_version
        
        # Add llm_graded field if it doesn't exist, default to False
        if 'llm_graded' not in meta:
            meta['llm_graded'] = False
    ...
    def upgrade_cell_metadata(self, cell: NotebookNode) -> NotebookNode:
        ...
        if cell.metadata['nbgrader']['schema_version'] == 0:
            cell = self.v3.v1._upgrade_v0_to_v1(cell)
            if 'nbgrader' not in cell.metadata:
                return cell

        if cell.metadata['nbgrader']['schema_version'] == 1:
            cell = self.v3.v2._upgrade_v1_to_v2(cell)

        if cell.metadata['nbgrader']['schema_version'] == 2:
            cell = self.v3._upgrade_v2_to_v3(cell)

        if cell.metadata['nbgrader']['schema_version'] == 3:
            cell = self._upgrade_v3_to_v4(cell)

        self._remove_extra_keys(cell)
        return cell

    def validate_cell(self, cell: NotebookNode) -> None:
        super(MetadataValidatorV4, self).validate_cell(cell)
        ...
        llm_graded = meta.get('llm_graded', False)
        ...
        # LLM graded cells must be grade AND solution
        if llm_graded:
            if not grade:
                raise ValidationError(
                    "LLM graded cell '{}' must be marked as a grade cell".format(
                        meta.get('grade_id', 'unknown')))
            if not solution:
                raise ValidationError(
                    "LLM graded cell '{}' must be marked as a solution cell".format(
                        meta.get('grade_id', 'unknown')))

        # check that markdown cells are grade AND solution (not either/or)
        if not task and not llm_graded:
...
def read_v4(source: typing.TextIO, as_version: int, **kwargs: typing.Any) -> NotebookNode:
    nb = _read(source, as_version, **kwargs)
    MetadataValidatorV4().validate_nb(nb)
    return nb


def write_v4(nb: NotebookNode, fp: typing.TextIO, **kwargs: typing.Any) -> None:
    MetadataValidatorV4().validate_nb(nb)
    _write(nb, fp, **kwargs)


def reads_v4(source: str, as_version: int, **kwargs: typing.Any) -> NotebookNode:
    nb = _reads(source, as_version, **kwargs)
    MetadataValidatorV4().validate_nb(nb)
    return nb


def writes_v4(nb: NotebookNode, **kwargs: typing.Any) -> None:
    MetadataValidatorV4().validate_nb(nb)
    _writes(nb, **kwargs)